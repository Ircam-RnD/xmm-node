...
void XmmWrap::addPhrase(const Nan::FunctionCallbackInfo<v8::Value> & args) {
	...
	//Nan::MaybeLocal<v8::Object> maybeInputPhrase = v8::Local<v8::Object>::Cast(args[0]);
...
void XmmWrap::getModel(const Nan::FunctionCallbackInfo<v8::Value> & args) {
	...
	switch(obj->modelType_) {
 	...
 	/*
	v8::Local<v8::Object> outputModel = Nan::New<v8::Object>();
	int nmodels = jm["models"].size();
	v8::Local<v8::Array> models = Nan::New<v8::Array>(nmodels);

    for(int i=0; i<nmodels; i++) {
        std::string l = jm["models"][i]["label"].asString();
        Nan::Set(models, i, Nan::New<v8::String>(l).ToLocalChecked());
    }
    */
	...
	// v8::Local<v8::Array> models = Nan::New<v8::Array>(nmodels);
	// for(int i=0; i<nmodels; i++) {
	// 	models->Set(i, Nan::New<v8::String>(jp["models"].get(i, "").asString()).ToLocalChecked());
	// }

	//Nan::Set(outputModel, Nan::New<v8::String>("models").ToLocalChecked(), models);

	//args.GetReturnValue().Set(outputModel);
